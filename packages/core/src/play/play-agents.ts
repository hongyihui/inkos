...
function buildWorldMutatorSystemPrompt(language: "zh" | "en"): string {
  if (language === "en") {
    return [
      ...
      "When the player holds/carries/keeps/takes a tangible thing, record an edge from actor_player to that entity and set value.role=\"holding\". The UI reads this canonical role, not the wording of type, so do not rely on type text alone.",
      ...
      "The following is only a JSON-shape example. Do not reuse its ids, labels, names, or story facts in the actual world:",
      `{"eventId":"evt-1","turn":1,"actionKind":"look","summary":"The player-character finds a sample clue and a sample key.","entities":{"upsert":[{"id":"actor_player","type":"actor","label":"player-character","summary":"Placeholder for the current world's player role; replace with the real roster id/label.","status":"alert","updatedEventId":"evt-1"},{"id":"actor_counterpart","type":"actor","label":"counterpart","summary":"Placeholder for a relevant person in the current world; replace with the real roster id/label.","status":"guarded","updatedEventId":"evt-1"},{"id":"evidence_sample_clue","type":"evidence","label":"sample clue","summary":"A tangible clue discovered this turn; replace with a real object from the scene.","status":"seen","updatedEventId":"evt-1"},{"id":"item_sample_key","type":"item","label":"sample key","summary":"A tangible item collected this turn; replace with a real object from the scene.","status":"collected","updatedEventId":"evt-1"}]},"edges":{"upsert":[{"fromId":"actor_player","type":"suspicious_of","toId":"actor_counterpart","value":{"role":"relation"}},{"fromId":"actor_player","type":"holds","toId":"item_sample_key","value":{"role":"holding"}}]},"stateSlots":{"upsert":[{"id":"slot_sample_timer","kind":"timer","label":"sample timer","value":3,"updatedEventId":"evt-1"}]}}`,
    ].join("\n");
  }
  return [
    "你是互动小说世界状态草案员。",
    ...
    "玩家持有/携带/收进包里/拿走某个实物时，必须记一条 actor_player 指向该实体的 edge，并设置 value.role=\"holding\"。侧栏只读这个规范 role，不靠 type 里的自然语言关系词猜。",
